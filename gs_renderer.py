...
class GaussianModel:
    ...
    def extract_mesh(self, path, density_thresh=1, resolution=128, decimate_target=1e5):
        ...
        #print(density_thresh,7777777777777777777777777777777777777777777777777777777777777777)
    ...
    def load_ply_gaosi(self, path):
        # 读取 PLY 文件
        plydata = PlyData.read(path)
        vertex_data = plydata['vertex'].data
        print(plydata,66666666666666666666666666)
        # 根据属性从 PLY 文件中提取数据
        ...
        f_dc = vertex_data['f_dc']
        f_rest = vertex_data['f_rest']
        opacities = vertex_data['opacities']
        scale = vertex_data['scale']
        rotation = vertex_data['rotation']

        # 将提取的数据分配给对象的属性
        self._xyz = torch.tensor(xyz).to(self._xyz.device).float()
        self._features_dc = torch.tensor(f_dc).view(self._features_dc.size()).to(self._features_dc.device).float()
        self._features_rest = torch.tensor(f_rest).view(self._features_rest.size()).to(
            self._features_rest.device).float()
        self._opacity = torch.tensor(opacities).to(self._opacity.device).float()
        self._scaling = torch.tensor(scale).to(self._scaling.device).float()
        self._rotation = torch.tensor(rotation).to(self._rotation.device).float()
