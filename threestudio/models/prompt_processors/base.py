...
class PromptProcessor(BaseObject):
    ...
    def configure(self) -> None:
        ...
        if self.cfg.use_prompt_debiasing:
            ...
            print(self.prompts_vd,777777777777777777777777777777777777)
    ...
    def load_text_embeddings(self):
        ...
        for prompt in self.prompts_vd:
            print(prompt,self.load_from_cache(prompt).shape)  # 打印出当前的prompt
        self.uncond_text_embeddings_vd = torch.stack(
            [self.load_from_cache(prompt) for prompt in self.negative_prompts_vd], dim=0
        )
        # for prompt in self.negative_prompts_vd:
        #     print(prompt,self.load_from_cache(prompt))  # 打印出当前的prompt
    ...
    def load_from_cache(self, prompt):
        ...
        print(prompt,cache_path)
        #print(cache_path,4445544466666666666666666666666666666666666666666)
