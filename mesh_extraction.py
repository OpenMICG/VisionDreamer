...
class GUI:
    ...
    def save_model(self, mode='geo', texture_size=1024):
        ...
        if mode == 'geo':
            path = os.path.join(self.opt.save_path + '_mesh.ply')
            #path = "/home/dongzeyi/gaosi/outputs/gaussiandreamer-sd/a_panda@20241029-091835/save/last_3dgs.ply"
            mesh = self.renderer.gaussians.extract_mesh(path, self.opt.density_thresh)
            mesh.write_ply(path)

        elif mode == 'geo+tex':
            path = os.path.join(self.opt.save_path + '_mesh.' + self.opt.mesh_format)
            ...
            path = os.path.join(self.opt.save_path + '_model.ply')

            with open('./configs/config.yaml', 'r') as file:
                params = yaml.safe_load(file)

            path_dir = str(params['param3'])

            self.renderer.gaussians.load_ply(path_dir)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config",  help="path to the yaml config file", default="configs/extration.yaml")
