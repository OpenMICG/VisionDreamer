...
class InstantMesh(nn.Module):
    ...
    def forward_planes(self, images, cameras):
        ...
        # text_feats = self.text_encoder(**inputs).last_hidden_state
        #
        # global_feature = text_feats[:, 0, :]
        #
        # num_chunks = 6
        # chunk_size = 401
        #
        # chunks = torch.chunk(image_feats, num_chunks, dim=1)
        #
        # reconstructed_features = torch.zeros_like(image_feats)
        #
        # for i, chunk in enumerate(chunks):
        #     cls_token = chunk[:, 0, :]
        #     combined_cls = cls_token + global_feature
        #     chunk[:, 0, :] = combined_cls
        #     reconstructed_features[:, i * chunk_size: (i + 1) * chunk_size, :] = chunk
